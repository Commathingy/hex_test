use bevy::{prelude::*, render::{render_resource::PrimitiveTopology, mesh::{Indices, shape::Plane}}, input::mouse::MouseMotion};
use hex_materials::OutlineMaterial;

mod hex_materials;


//to avoic z fighting on overlap:
//custom material overriding depth_bias() method: https://docs.rs/bevy/latest/bevy/pbr/trait.Material.html#method.depth_bias
...
fn main() {
    App::new()
        ...
        .add_systems(PreStartup, add_hex_mesh)
        .add_systems(Startup, (spawn_hexes, spawn_camera))
...
fn spawn_hexes(
    ...
    commands.spawn(
        MaterialMeshBundle{
            ...
            material: handles.hex_material.clone(),
            ..default()
        }
    );
...
fn camera_move(
    ...
    let total_mouse_movement = -0.003 * mouse_input.read().fold(Vec2::ZERO, |acc, val| acc + val.delta);
    camera_t.rotate_y(total_mouse_movement.x);
    camera_t.rotate_local_x(total_mouse_movement.y);

    let forward = 0.1 * camera_t.forward();
    let left = 0.1 * camera_t.left();

    if input.pressed(KeyCode::A){
        camera_t.translation += left;
    }
    if input.pressed(KeyCode::D){
        camera_t.translation -= left;
    }
    if input.pressed(KeyCode::W){
        camera_t.translation += forward;
    }
